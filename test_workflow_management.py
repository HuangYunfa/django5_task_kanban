...
def test_workflow_features():
    ...
    # 检查状态类型分布
    initial_statuses = WorkflowStatus.objects.filter(is_initial=True).count()
    final_statuses = WorkflowStatus.objects.filter(is_final=True).count()
    active_statuses = WorkflowStatus.objects.filter(is_active=True).count()
    
    print(f"   📊 状态类型分布:")
    print(f"      • 初始状态: {initial_statuses} 个")
    print(f"      • 最终状态: {final_statuses} 个")
    print(f"      • 活跃状态: {active_statuses} 个")
    
    # 检查转换规则特性
    auto_notify = WorkflowTransition.objects.filter(auto_notify_assignees=True).count()
    require_comment = WorkflowTransition.objects.filter(require_comment=True).count()
    require_assignee = WorkflowTransition.objects.filter(require_assignee=True).count()
    
    print(f"   🔄 转换规则特性:")
    print(f"      • 自动通知: {auto_notify} 个")
    print(f"      • 需要备注: {require_comment} 个")
    print(f"      • 需要受理人: {require_assignee} 个")
