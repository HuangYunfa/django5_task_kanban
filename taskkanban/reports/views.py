...
from .services import ReportDataService, ChartDataService
...
class ExportChartDataView(LoginRequiredMixin, TemplateView):
    """导出图表数据视图"""
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            data_type = request.POST.get('data_type', 'tasks')
            ...
            if data_type == 'tasks':
                ...
                chart_data = ChartDataService.format_for_chartjs(
                    raw_data.get('status_stats', []), 
                    chart_type
                )
            elif data_type == 'workload':
                raw_data = data_service.get_user_workload_stats()
                chart_data = ChartDataService.format_for_chartjs(
                    raw_data.get('user_workloads', []), 
                    chart_type
                )
            elif data_type == 'teams':
                raw_data = data_service.get_team_performance_stats()
                chart_data = ChartDataService.format_for_chartjs(
                    raw_data.get('team_stats', []), 
                    chart_type
                )
            else:
            ...
            # 导出图表数据
            return ChartExportService.export_chart_data(chart_data)
